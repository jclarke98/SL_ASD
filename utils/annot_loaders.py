...
def load_manifest(clip_ids, annotation_path, bbox_path, fps = 30,
    ...
    # eliminate masking_prob percent of the utterances in utt_id2concurrent_tracks randomly
    if masking_prob > 0:
        utt_id2concurrent_tracks_ = {}
        utt_ids = list(utt_id2concurrent_tracks.keys())
        num_to_mask = int(len(utt_ids) * (1-masking_prob))
        utt_ids = random.sample(utt_ids, num_to_mask)
        for utt_id in utt_ids:
            utt_id2concurrent_tracks_[utt_id] = utt_id2concurrent_tracks[utt_id]
        return utt_id2concurrent_tracks_
    else:
        return utt_id2concurrent_tracks

# def load_manifest(clip_ids, annotation_path, bbox_path, fps = 30, masking_prob=0.0):
#     """
#     Load audio manifests from the specified directory and the bbox data 
#     to return a dictionary of meta data regarding trackids concurrent to each utterance.
